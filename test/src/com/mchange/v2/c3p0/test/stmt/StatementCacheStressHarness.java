...
public final class StatementCacheStressHarness
{
    ...
    private final static class Worker implements Runnable
    {
        ...
        /**
         * Mutates Statement state the way a client would.
         *
         * <p>On 0.15.x this method also reports each mutation to the cache, via
         * markCursorNameSet(...), markMaxRowsUpdatedFrom(...) and friends, as the Statement
         * proxies do -- which drives the Hazards machinery, and, for the irreversible mutations,
         * makes checkin discard the Statement through removeStatement( ps, DESTROY_ALWAYS ).
         *
         * <p>0.14.x has no Hazards machinery: refreshStatement(...) is clearParameters() and
         * clearBatch(), and nothing tracks mutated Statement state. So here the mutations stand
         * alone. Do not "restore" the mark... calls on this branch; they do not exist. What this
         * still exercises is a client mutating a Statement that the cache will hand to somebody
         * else -- and that 0.14.x does not put it back is a separate, known matter, fixed on
         * 0.15.x and covered there by mill test.c3p0StatementState.
         */
